...
-- 
-- ⚠️ חשוב: Migration זה מעדכן את הטבלה subscriptions הקיימת
-- אם יש רשומות קיימות, הן לא יושפעו (רק המבנה משתנה)
-- ============================================================================
...
-- ============================================================================
-- ⚠️ עדכון הטבלה הקיימת subscriptions
-- אם הטבלה לא קיימת, היא תיווצר
-- אם היא קיימת, נוסיף/נעדכן עמודות לפי הצורך
...
BEGIN
  ...
  ELSE
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'subscriptions' AND column_name = 'minutes_used_monthly') THEN
      ALTER TABLE public.subscriptions 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'subscriptions' AND column_name = 'analyses_used_monthly') THEN
      ALTER TABLE public.subscriptions 
      ADD COLUMN analyses_used_monthly integer NOT NULL DEFAULT 0 
        CHECK (analyses_used_monthly >= 0);
    END IF;
    
    -- עדכן את user_id להיות Primary Key אם לא
    -- ⚠️ זה מסוכן אם יש כבר Primary Key אחר - צריך לבדוק ידנית
...
-- מחיקת policies קיימות (אם יש) - רק את אלה שלנו
